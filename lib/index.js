...
let SkillManagerGateway = (() => {
  ...
  return class SkillManagerGateway extends _classSuper {
    static {
      ...
      _rename_decorators = [Remote("skillRename")];
      ...
      __esDecorate(this, null, _rename_decorators, {
        kind: "method", name: "skillRename", static: false, private: false,
        access: { has: (obj) => "skillRename" in obj, get: (obj) => obj.skillRename },
        metadata: _metadata
      }, null, _instanceExtraInitializers);
    ...
    async skillUpdate(spec) {
      ...
      // Keep the frontmatter `name` in sync with the directory name (covers the
      // rename-then-update flow, where the content may still carry the old name).
      const fm = extractFrontmatter(text);
      if (fm !== null && /^\s*name\s*:/m.test(fm.body)) {
        const body = fm.body.replace(/^(\s*name\s*:\s*).*$/m, `$1${name}`);
        if (body !== fm.body) await writeFile(skillFile, `---\n${body}${text.slice(fm.end)}`, "utf8");
      }
    ...
    /**
     * Rename a user-level skill: move its directory to the new name and sync
     * the `name` field inside SKILL.md frontmatter (when present).
     */
    async skillRename(spec) {
      const { name, newName } = spec;
      if (typeof name !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name)) throw new Error("skill-manager: invalid skill name");
      if (typeof newName !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(newName)) throw new Error("skill-manager: invalid new skill name (lowercase letters, digits, dashes)");
      if (name === newName) return { ok: true, name, newName };
      const dir = profileSkillsDir(this.ctx);
      const from = join(dir, name);
      const to = join(dir, newName);
      try {
        await rm(to, { recursive: true, force: false });
      } catch (error) {
        if (error?.code !== "ENOENT") throw error;
      }
      // Windows: the skill watcher may briefly hold the directory, so retry a
      // plain rename and fall back to copy+delete if it keeps failing.
      let moved = false;
      let lastError = null;
      ...
      if (!moved) {
        if (lastError?.code === "ENOENT") throw new Error(`skill-manager: skill ${name} not found`);
        try {
      ...
      if (!moved) throw lastError;
      // Sync the frontmatter `name` field so the display name matches the directory.
      const skillFile = join(to, "SKILL.md");
      try {
        const text = await readFile(skillFile, "utf8");
        const fm = extractFrontmatter(text);
        if (fm !== null && /^\s*name\s*:/m.test(fm.body)) {
          const edited = fm.body.replace(/^(\s*name\s*:\s*).*$/m, `$1${newName}`);
          if (edited !== fm.body) await writeFile(skillFile, `---\n${edited}${text.slice(fm.end)}`, "utf8");
        }
      } catch { /* unreadable SKILL.md: keep the moved directory as-is */ }
